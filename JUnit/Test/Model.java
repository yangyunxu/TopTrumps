...
public class Model {
    ...
    //mock函数，按顺序添加卡牌
  	public void distributeCardsMock(File file)  {
        BufferedReader reader0 = null;     
        List<Card> cardList0 = new ArrayList<Card>(); 
        try {
        	//将file中的牌读取到cardlist0中
            reader0 = new BufferedReader(new FileReader(file));  
            String line;
            reader0.readLine();                                   
                                                                
            while ((line = reader0.readLine()) != null){
                cardList0.add(new Card(line));
            }
            Iterator<Player> it = players.iterator();
            for(Card c:cardList0){
                if(it.hasNext()){    
                    it.next().add(c);
                }else {
                    it = players.iterator(); 
